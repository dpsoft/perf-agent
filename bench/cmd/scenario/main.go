...
func measureOnePID(pid, runN int, unwind string) schema.Run {
	...
	t0 := time.Now()
	prof, err := dwarfagent.NewProfilerWithMode(pid, false, []uint{0}, nil, 99, hooks, modeFromFlag(unwind), nil, nil, nil, sym, false)
...
func measureSystemWide(runN int, unwind string) schema.Run {
	...
	t0 := time.Now()
	prof, err := dwarfagent.NewProfilerWithMode(0, true, cpus, nil, 99, hooks, modeFromFlag(unwind), nil, nil, nil, sym, false)
	totalMs := float64(time.Since(t0).Microseconds()) / 1000.0
	if err != nil {
