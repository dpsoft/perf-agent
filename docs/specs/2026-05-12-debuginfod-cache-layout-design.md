...
- **DWARF unwinder support for deleted binaries**: when a target process's binary is deleted from disk while still mapped (sidecar / mount-namespace case), `unwind/dwarfagent` fails to attach because it opens the binary by symbolic path to compile `.eh_frame`. The agent has caps to open `/proc/<pid>/map_files/<range>` instead. Extending the DWARF unwinder's attach path to prefer `map_files` would close this gap. Separate PR — touches a different subsystem.
