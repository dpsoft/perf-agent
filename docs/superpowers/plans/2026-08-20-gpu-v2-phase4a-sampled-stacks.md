...
- **Symbolizing a process that has already exited.** A sampled stack is symbolized in `Consumer.resolveStackLocked` against `/proc/<pid>/maps` of the *launching* process, which the kernel destroys the moment that process exits. Phase 4a leaves this unsolved and only *counts* it: `Stats.StacksUnresolved` is the number of captures in which not one frame resolved to a name, and `Stats.StackFramesUnresolved` the per-frame version. For a long-running GPU process this never fires — the consumer drains continuously and the producer outlives its records by orders of magnitude. **For a short-lived one it will**, and that is precisely the workload the real CUPTI adapter meets: a process that launches a handful of kernels and exits can beat the drain, and every one of its stacks then renders as `0x4017c2` in the profile. The Phase 4a gate hit exactly this shape and it took a human reading hex to notice, because at the time nothing counted it.
