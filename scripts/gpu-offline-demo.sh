...
Usage:
  ...
  scripts/gpu-offline-demo.sh [--dry-run] hip-rocprofiler-sdk-llm-rich <outdir>
...
Modes:
  ...
  hip-rocprofiler-sdk-llm-rich checked-in model-style host->rocprofiler-sdk->collector->AMD sample path with deeper CPU frames and richer algorithm GPU frames
...
case "${MODE}" in
    ...
    hip-rocprofv3-rich)
        ...
        AMD_SAMPLE_SOURCE_PATH_ENV="PERF_AGENT_ROCPROFV3_PATH"
        ...
        NAME="rocprofv3_sample_exec_rich"
        EXTRA_ARGS=("--gpu-amd-sample-stdin")
        ;;
    hip-rocprofv3-command-rich)
        HOST_REPLAY="gpu/testdata/host/replay/hip_kfd_launches.json"
        AMD_SAMPLE_SOURCE_COMMAND='scripts/emit-rocprofv3-rich-fixture.sh > "$PERF_AGENT_ROCPROFV3_OUTPUT_PATH"'
        AMD_SAMPLE_SOURCE_REAL_SOURCE="rocprofv3"
    ...
    hip-rocprofiler-sdk-llm-rich)
        HOST_REPLAY="gpu/testdata/host/replay/llm_flash_attn_launches.json"
        AMD_SAMPLE_SOURCE_COMMAND='cat gpu/testdata/replay/rocprofiler_sdk_native_llm_rich.ndjson > "$PERF_AGENT_ROCPROFILER_SDK_OUTPUT_PATH"'
        AMD_SAMPLE_SOURCE_REAL_SOURCE="rocprofiler-sdk"
        AMD_SAMPLE_SOURCE_COMMAND_ENV="PERF_AGENT_ROCPROFILER_SDK_COMMAND"
        AMD_SAMPLE_SOURCE_OUTPUT_ENV="PERF_AGENT_ROCPROFILER_SDK_OUTPUT_PATH"
        AMD_SAMPLE_SOURCE_OUTPUT_FILE="${OUTDIR}/rocprofiler_sdk_native_llm_rich.ndjson"
        NAME="rocprofiler_sdk_llm_sample_exec_rich"
        EXTRA_ARGS=("--gpu-amd-sample-stdin")
        ;;
    ...
    hip-rocprofiler-sdk-recorder-rich)
        HOST_REPLAY="gpu/testdata/host/replay/hip_kfd_launches.json"
        AMD_SAMPLE_SOURCE_COMMAND='cat gpu/testdata/replay/rocprofiler_sdk_native_rich.json > "$PERF_AGENT_ROCPROFILER_SDK_OUTPUT_PATH"'
        AMD_SAMPLE_SOURCE_REAL_SOURCE="rocprofiler-sdk"
        AMD_SAMPLE_SOURCE_COMMAND_ENV="PERF_AGENT_ROCPROFILER_SDK_COMMAND"
        AMD_SAMPLE_SOURCE_OUTPUT_ENV="PERF_AGENT_ROCPROFILER_SDK_OUTPUT_PATH"
        AMD_SAMPLE_SOURCE_OUTPUT_FILE="${OUTDIR}/rocprofiler_sdk_native_rich.json"
        NAME="rocprofiler_sdk_recorder_sample_exec_rich"
        EXTRA_ARGS=("--gpu-amd-sample-stdin")
        ;;
...
if [[ ${#AMD_SAMPLE_COLLECTOR_CMD[@]} -eq 0 && -n "${AMD_SAMPLE_SOURCE_PATH}" ]]; then
    AMD_SAMPLE_COLLECTOR_CMD=(
        ...
        "GOMODCACHE=${GOMODCACHE:-/tmp/perf-agent-gomodcache}"
        "GOTOOLCHAIN=${GOTOOLCHAIN:-auto}"
        "${AMD_SAMPLE_SOURCE_PATH_ENV}=${REPO_ROOT}/${AMD_SAMPLE_SOURCE_PATH}"
